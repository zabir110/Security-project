...
namespace FileEncryptor
{
    ...
    public class opensslkey
    {
        ...
        private static byte[] GetPkcs12(RSA rsa, String keycontainer, String cspprovider, uint KEYSPEC, uint cspflags)
        {
            ...
            rsa.Clear();
            if (hCertCntxt != IntPtr.Zero)
                FileEncryptor.KeyConverter.CertFreeCertificateContext(hCertCntxt);
            return pfxblob;
        }
        ...
        private static IntPtr CreateUnsignedCertCntxt(String keycontainer, String provider, uint KEYSPEC, uint cspflags, String DN)
        {
            ...
            if (FileEncryptor.KeyConverter.CertStrToName(X509_ASN_ENCODING, DN, CERT_X500_NAME_STR, IntPtr.Zero, null, ref cbName, IntPtr.Zero))
            {
                encodedName = new byte[cbName];
                FileEncryptor.KeyConverter.CertStrToName(X509_ASN_ENCODING, DN, CERT_X500_NAME_STR, IntPtr.Zero, encodedName, ref cbName, IntPtr.Zero);
            }
            ...
            hCertCntxt = FileEncryptor.KeyConverter.CertCreateSelfSignCertificate(IntPtr.Zero, ref subjectblob, CERT_CREATE_SELFSIGN_NO_SIGN, ref pInfo, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero);
            if (hCertCntxt == IntPtr.Zero)
